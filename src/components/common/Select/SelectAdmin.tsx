...
// import Select, { type SingleValue, type ActionMeta, type StylesConfig, type DropdownIndicatorProps, components } from 'react-select';
...
// const CustomSelectAdmin: React.FC<CustomSelectAdminProps> = ({ required, className = '' }) => {
...
// components/common/Select/SelectAdmin.tsx
import React, { useState } from 'react';
...
// Маппинг значений для chapter
const chapterMap: Record<string, 'svo' | 'gpw'> = {
    'Герой СВО': 'svo',
    'Герой СССР': 'gpw',
};

// Маппинг значений для cardType
...
const CustomSelectAdmin: React.FC<CustomSelectAdminProps> = ({
    ...
    const handleChangeHero = (newValue: SingleValue<OptionType>): void => {
        setHero(newValue);
        // Вызываем колбэк с преобразованным значением
        if (onChapterChange) {
